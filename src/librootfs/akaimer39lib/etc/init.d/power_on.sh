...
if iwlist wlan0 scan | grep -i $AP 1>/dev/null ; then
	...
	Signal=$(iwconfig wlan0 | grep Signal | cut -d "-" -f 2 | cut -d " " -f 1)
		if [ $Signal -le 70 ] > /dev/null; then
			...
			# copy on.txt
			rsync -avm --no-o --no-g --remove-source-files --password-file=/etc/.rsync /etc/on_`hostname`.txt root@$Server::video/Avtobus/`date +%Y%m%d`/
			# copy main files
			...
			# copy log file
			rsync -avm --no-o --no-g --remove-source-files --password-file=/etc/.rsync /etc/`hostname`.txt root@$Server::video/Avtobus/`date +%Y%m%d`/
